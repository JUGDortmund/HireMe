...
public class ProfileTest extends NinjaDocTester {
  ...
  public void addProfileReturnsProfile() throws Exception {
    ...
    assertThat(response.payloadAs(Profile.class).getFirstname()).isEqualTo("Klaus");
  }
  ...
  public void addingAProfileIncreasesSizeOfProfileList() throws Exception {
    Response firstResponse = sayAndMakeRequest(
        Request.GET().url(
            testServerUrl().path("/api/profile")
        )
    );
    ...
    Response secondResponse = sayAndMakeRequest(
        Request.GET().url(
            testServerUrl().path("/api/profile")
        )
    );
